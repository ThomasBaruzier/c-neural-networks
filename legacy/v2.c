...
// TO DO
// Handle data being split by fread
int loadData(FILE *fp, struct Dataset *ds, struct HyperParams *hp) {
  ...
  while (fread(buffer, 1, 1024*1024, fp) > 0) {
    for (int pos=0; line < 2 && buffer[pos] != '\0'; pos++) {
      ...
      if (line == 0)  {
        // reallocate inputs
        if (set == arraySize) {
          if (arraySize * numNeuronsIn * sizeof(float) < 1024 * 1024 * 8) {
            arraySize *= 2;
          } else {
            arraySize += 1024 * 1024 * 2 / numNeuronsIn;
          }
          printf("Reallocating inputs[] by %d (%lu bytes)\n", arraySize, arraySize * sizeof(float *));
          printf("Allowing memory for inputs[][] (%lu bytes)\n", arraySize * numNeuronsIn * sizeof(float));
        ...
        // reallocate outputs
        if (set == arraySize) {
          if (arraySize * numNeuronsOut * sizeof(float) < 1024 * 1024 * 8) {
            arraySize *= 2;
          } else {
            arraySize += 1024 * 1024 * 2 / numNeuronsOut;
          }
          printf("Reallocating outputs[] by %d (%lu bytes)\n", arraySize, arraySize * sizeof(float *) + 1);
          printf("Allowing memory for outputs[][] (%lu bytes)\n", arraySize * numNeuronsOut * sizeof(float *));
          ds->outputs = (float **) realloc(ds->outputs, arraySize * sizeof(float *) + 1);
          outputData = (float *) realloc(outputData, arraySize * numNeuronsOut * sizeof(float));
        }
